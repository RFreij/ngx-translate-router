...
export abstract class LocalizeParser {
    ...
    protected init(routes: Routes): Promise<any> {
        ...
        if (this.settings.defaultLangFunction) {
            ...
            this.defaultLang =
                this._cachedLang || browserLang || this.locales[0];
        }
        ...
        for (let i = children.length - 1; i >= 0; i--) {
            if (children[i].data && children[i].data['skipRouteLocalization']) {
                ...
                if (
                    ...
                    !children[i].data['skipRouteLocalization'][
                        'localizeRedirectTo'
                    ]
                ) {
                    children.splice(i, 1);
    ...
    private _translateRouteTree(routes: Routes): void {
        routes.forEach((route: Route) => {
            ...
            const localizeRedirection =
                !skipRouteLocalization ||
                skipRouteLocalization['localizeRedirectTo'];
    ...
    private _translateProperty(
        ...
        (<any>route)[property] = prefixLang
            ? this.addPrefixToUrl(result)
            : result;
    }
    ...
    getLocationLang(url?: string): string {
        ...
        if (
            pathSlices.length > 1 &&
            this.locales.indexOf(pathSlices[1]) !== -1
        ) {
            return pathSlices[1];
        }
